...
// Package compiler wraps the Ylem executable (ylem).
package compiler
...
// Ylem contains information about the ylem compiler.
type Ylem struct {
...
// ylem v.0.8 changes the way ABI, Devdoc and Userdoc are serialized
...
// YlemVersion runs ylem and parses its version output.
func YlemVersion(ylem string) (*Ylem, error) {
	if ylem == "" {
		ylem = "ylem"
	}
	...
	s := &Ylem{Path: cmd.Path, FullVersion: out.String(), Version: matches[0]}
...
// CompileYlem compiles all given Ylem source files.
...
func (s *Ylem) run(cmd *exec.Cmd, source string) (map[string]*Contract, error) {
...
// and parses it using the rules from ylem v.0.8.0 and later.
