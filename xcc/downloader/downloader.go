...
var (
	...
	maxQueuedHeaders         = 32 * 1024                    // [xcc/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	stateDB    xccdb.Database  // Database to state sync into (and deduplicate via)
	...
	// Channels
	headerCh      chan dataPack        // [xcc/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [xcc/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [xcc/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [xcc/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [xcc/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [xcc/62] Channel to feed the header processor new tasks

	// for stateFetcher
	stateSyncStart chan *stateSync
	trackStateReq  chan *stateReq
	stateCh        chan dataPack // [xcc/63] Channel receiving inbound node state data
...
func New(checkpoint uint64, stateDb xccdb.Database, stateBloom *trie.SyncBloom, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
// these are zero.
func (d *Downloader) Progress() core.SyncProgress {
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "xcc", p.version, "head", hash, "td", td, "mode", d.mode)
