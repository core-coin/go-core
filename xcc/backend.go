...
func New(ctx *node.ServiceContext, config *Config) (*Core, error) {
	...
	xcc.blockchain, err = core.NewBlockChain(chainDb, cacheConfig, chainConfig, xcc.engine, vmConfig, xcc.shouldPreserve)
	if err != nil {
		return nil, err
	}
	...
	if compat, ok := genesisErr.(*params.ConfigCompatError); ok {
		...
		xcc.blockchain.SetHead(compat.RewindTo)
		rawdb.WriteChainConfig(chainDb, genesisHash, chainConfig)
	}
	xcc.bloomIndexer.Start(xcc.blockchain)

	if config.TxPool.Journal != "" {
		config.TxPool.Journal = ctx.ResolvePath(config.TxPool.Journal)
	}
	xcc.txPool = core.NewTxPool(config.TxPool, chainConfig, xcc.blockchain)
	...
	if xcc.protocolManager, err = NewProtocolManager(chainConfig, checkpoint, config.SyncMode, config.NetworkId, xcc.eventMux, xcc.txPool, xcc.engine, xcc.blockchain, chainDb, cacheLimit, config.Whitelist); err != nil {
		return nil, err
	}
	xcc.miner = miner.New(xcc, &config.Miner, chainConfig, xcc.EventMux(), xcc.engine, xcc.isLocalBlock)
	xcc.miner.SetExtra(makeExtraData(config.Miner.ExtraData))

	xcc.APIBackend = &XccAPIBackend{ctx.ExtRPCEnabled(), xcc, nil}
	gpoParams := config.GPO
	if gpoParams.Default == nil {
		gpoParams.Default = config.Miner.EnergyPrice
	}
	xcc.APIBackend.gpo = energyprice.NewOracle(xcc.APIBackend, gpoParams)

	xcc.dialCandiates, err = xcc.setupDiscovery(&ctx.Config.P2P)
	if err != nil {
		return nil, err
	}
...
func CreateConsensusEngine(ctx *node.ServiceContext, chainConfig *params.ChainConfig, config *cryptore.Config, notify []string, noverify bool, db xccdb.Database) consensus.Engine {
...
func (s *Core) ChainDb() xccdb.Database            { return s.chainDb }
func (s *Core) IsListening() bool                  { return true } // Always listening
func (s *Core) XccVersion() int                    { return int(ProtocolVersions[0]) }
...
func (s *Core) Protocols() []p2p.Protocol {
	...
	for i, vsn := range ProtocolVersions {
		...
		protos[i].Attributes = []enr.Entry{s.currentXccEntry()}
...
func (s *Core) Start(srvr *p2p.Server) error {
	s.startXccEntryUpdate(srvr.LocalNode())

	// Start the bloom bits servicing goroutines
	s.startBloomHandlers(params.BloomBitsBlocks)

	// Start the RPC service
	s.netRPCService = xccapi.NewPublicNetAPI(srvr, s.NetVersion())
