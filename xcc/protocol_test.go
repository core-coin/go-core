...
func TestForkIDSplit(t *testing.T) {
	var (
		...
		xccNoFork, _  = NewProtocolManager(configNoFork, nil, downloader.FullSync, 1, new(event.TypeMux), &testTxPool{pool: make(map[common.Hash]*types.Transaction)}, engine, chainNoFork, dbNoFork, 1, nil)
		xccProFork, _ = NewProtocolManager(configProFork, nil, downloader.FullSync, 1, new(event.TypeMux), &testTxPool{pool: make(map[common.Hash]*types.Transaction)}, engine, chainProFork, dbProFork, 1, nil)
	)
	xccNoFork.Start(1000)
	xccProFork.Start(1000)
	...
	go func() { errc <- xccNoFork.handle(peerProFork) }()
	go func() { errc <- xccProFork.handle(peerNoFork) }()
