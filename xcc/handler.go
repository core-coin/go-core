...
func NewProtocolManager(config *params.ChainConfig, checkpoint *params.TrustedCheckpoint, mode downloader.SyncMode, networkID uint64, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb xccdb.Database, cacheLimit int, whitelist map[uint64]common.Hash) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) Start(maxPeers int) {
	...
	go pm.txsyncLoop64() // TODO(raisty): Legacy initial tx echange, drop with xcc/64.
}
...
// handle is the callback invoked to manage the life cycle of an xcc peer. When
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case p.version >= xcc63 && msg.Code == GetNodeDataMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= xcc63 && msg.Code == NodeDataMsg:
	...
	case p.version >= xcc63 && msg.Code == GetReceiptsMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= xcc63 && msg.Code == ReceiptsMsg:
	...
	case msg.Code == NewPooledTransactionHashesMsg && p.version >= xcc65:
	...
	case msg.Code == GetPooledTransactionsMsg && p.version >= xcc65:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case msg.Code == TransactionMsg || (msg.Code == PooledTransactionsMsg && p.version >= xcc65):
