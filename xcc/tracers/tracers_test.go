...
var makeTest = function(tx, rewind) {
  ...
  var block   = xcc.getBlock(xcc.getTransaction(tx).blockHash);
  var genesis = xcc.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.xcc.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  xcc.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
...
func TestPrestateTracerCreate2(t *testing.T) {
	addr, err := common.HexToAddress("cb5700000000000000000000000000000000deadbeef")
	if err != nil {
		t.Error(err)
	}
	...
	*/
	...
	cvm := vm.NewCVM(context, statedb, params.MainnetChainConfig, vm.Config{Debug: true, Tracer: tracer})
	...
	if _, has := ret["0xcb497c29a2505dfc709318c1794c18a31f1dafa4"]; !has {
		t.Fatalf("Expected 0xcb497c29a2505dfc709318c1794c18a31f1dafa4 in result")
	}
}
