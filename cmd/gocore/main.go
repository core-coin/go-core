...
var (
	...
	app = flags.NewApp(gitTag, gitCommit, gitDate, "the go-core command line interface")
...
func init() {
	...
	app.Copyright = "Copyright 2023 The CORE FOUNDATION, nadacia"
...
func prepare(ctx *cli.Context) {
	...
	switch {
	case ctx.GlobalIsSet(utils.DevinFlag.Name):
		log.Info("Starting Gocore on Devin testnet...")

	case ctx.GlobalIsSet(utils.DeveloperFlag.Name):
		log.Info("Starting Gocore in ephemeral dev mode...")

	case !ctx.GlobalIsSet(utils.NetworkIdFlag.Name):
		log.Info("Starting Gocore on Core mainnet...")
	}
	// If we're a full node on mainnet without --cache specified, bump default cache allowance
	if ctx.GlobalString(utils.SyncModeFlag.Name) != "light" && !ctx.GlobalIsSet(utils.CacheFlag.Name) && !ctx.GlobalIsSet(utils.NetworkIdFlag.Name) {
		// Make sure we're not on any supported preconfigured testnet either
		if !ctx.GlobalIsSet(utils.DevinFlag.Name) && !ctx.GlobalIsSet(utils.DeveloperFlag.Name) {
