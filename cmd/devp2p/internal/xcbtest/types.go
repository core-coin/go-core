...
// Status is the network packet for the status message for xcb/64 and later.
type Status struct {
...
type Conn struct {
	*rlpx.Conn
	ourKey             *crypto.PrivateKey
	xcbProtocolVersion uint
}
...
func (c *Conn) handshake(t *utesting.T) Message {
	...
	case *Hello:
		...
		c.negotiateXcbProtocol(msg.Caps)
		if c.xcbProtocolVersion == 0 {
			t.Fatalf("unexpected xcb protocol version")
		}
		return msg
	default:
...
// negotiateXcbProtocol sets the Conn's xcb protocol version
// to highest advertised capability from peer
func (c *Conn) negotiateXcbProtocol(caps []p2p.Cap) {
	var highestXcbVersion uint
	for _, capability := range caps {
		if capability.Name != "xcb" {
			continue
		}
		if capability.Version > highestXcbVersion && capability.Version <= 65 {
			highestXcbVersion = capability.Version
		}
	}
	c.xcbProtocolVersion = highestXcbVersion
}
...
loop:
	...
	// make sure xcb protocol version is set for negotiation
	if c.xcbProtocolVersion == 0 {
		t.Fatalf("xcb protocol version must be set in Conn")
	}
	if status == nil {
		// write status message to client
		status = &Status{
			ProtocolVersion: uint32(c.xcbProtocolVersion),
			NetworkID:       chain.chainConfig.NetworkID.Uint64(),
