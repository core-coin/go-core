...
// Suite represents a structure used to test the xcb
...
// NewSuite creates and returns a new xcb-test suite that can
...
func (s *Suite) TestMaliciousStatus(t *utesting.T) {
	...
	conn.handshake(t)
	status := &Status{
		ProtocolVersion: uint32(conn.xcbProtocolVersion),
		NetworkID:       s.chain.chainConfig.NetworkID.Uint64(),
		TD:              largeNumber(2),
...
func (s *Suite) dial() (*Conn, error) {
	...
	// do encHandshake
	conn.ourKey, _ = crypto.GenerateKey(crand.Reader)
