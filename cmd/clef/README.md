...
Clef can run as a daemon on the same machine or even a separate VM in a [QubesOS](https://www.qubes-os.org/) type setup.
...
GLOBAL OPTIONS:
   ...
   --networkid value       Chain id to use for signing (1=mainnet, 3=Testnet, 4=Enterprise) (default: 1)
...
Clef listens to HTTP requests on `http.addr`:`http.port` (or to IPC on `ipcpath`), with the same JSON-RPC standard as Gocore. The messages are expected to be [JSON-RPC 2.0 standard](https://www.jsonrpc.org/specification).
...
The signer will generate a new private key, encrypt it according to [web3 keystore spec](https://github.com/core/wiki/wiki/Web3-Secret-Storage-Definition) and store it in the keystore directory.  
...
#### Arguments
  - content type [string]: type of signed data
     ...
     - `application/clique`: [clique](https://github.com/core/CIPs/issues/225) headers
...
#### Sign data
   Signs a chunk of structured data conformant to [CIP-712](https://github.com/core/CIPs/blob/master/CIPS/cip-712.md) and returns the calculated signature.
...
```json
{
  ...
  "params": [
    "cb76a631db606f1452ddc2432931d611f1d5b126f848",
    {
      "types": {
        "CIP712Domain": [
          {
