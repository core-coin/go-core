...
func deploy(ctx *cli.Context) error {
	...
	for _, account := range strings.Split(ctx.String(signersFlag.Name), ",") {
		...
		if err != nil {
			utils.Fatalf("invalid account %v, err: %v", addr, err)
		}
...
func sign(ctx *cli.Context) error {
	...
	if !ctx.GlobalIsSet(nodeURLFlag.Name) {
		...
		address, err = common.HexToAddress(ctx.String(oracleFlag.Name))
		if err != nil {
			utils.Fatalf("invalid account %v, err: %v", ctx.String(oracleFlag.Name), err)
		}
	} else {
	...
	if err != nil {
		utils.Fatalf("invalid account %v, err: %v", signer, err)
	}
	if !offline {
...
func publish(ctx *cli.Context) error {
	...
	for _, sig := range strings.Split(ctx.String(signaturesFlag.Name), ",") {
		...
		if len(trimmed) != crypto.ExtendedSignatureLength*2 {
