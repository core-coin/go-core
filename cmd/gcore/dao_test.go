...
func testDAOForkBlockNewChain(t *testing.T, test int, genesis string, expectBlock *big.Int, expectVote bool) {
	...
	// Start a Gcore instance with the requested flags set and immediately terminate
	if genesis != "" {
		...
		gcore := runGcore(t, append(args, []string{"--exec", "2+2", "console"}...)...)
		gcore.WaitExit()
	}
	// Retrieve the DAO config flag from the database
	path := filepath.Join(datadir, "gcore", "chaindata")
