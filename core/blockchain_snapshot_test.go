...
// (i)   Gocore restarts normally with valid legacy snapshot
// (ii)  Gocore restarts normally with valid new-format snapshot
// (iii) Gocore restarts after the crash, with broken legacy snapshot
// (iv)  Gocore restarts after the crash, with broken new-format snapshot
// (v)   Gocore restarts normally, but it's requested to be rewound to a lower point via SetHead
// (vi)  Gocore restarts normally with a stale snapshot
type snapshotTest struct {
	legacy       bool   // Flag whether the loaded snapshot is in legacy format
	crash        bool   // Flag whether the Gocore restarts from the previous crash
...
// Tests a Gocore restart with valid snapshot. Before the shutdown, all snapshot
...
// Tests a Gocore restart with valid but "legacy" snapshot. Before the shutdown,
...
// Tests a Gocore was crashed and restarts with a broken snapshot. In this case the
...
// Tests a Gocore was crashed and restarts with a broken snapshot. In this case
...
// Tests a Gocore was crashed and restarts with a broken and "legacy format"
...
// Tests a Gocore was running with snapshot enabled. Then restarts without
...
// Tests a Gocore was running with leagcy snapshot enabled. Then restarts
...
// Tests the Gocore was running with snapshot enabled and resetHead is applied.
...
// Tests the Gocore was running with snapshot(legacy-format) enabled and resetHead
...
// Tests the Gocore was running with snapshot(legacy-format) enabled and upgrades
// the disk layer journal(journal generator) to latest format. After that the Gocore
// is restarted from a crash. In this case Gocore will find the new-format disk layer
...
func testSnapshot(t *testing.T, tt *snapshotTest) {
	...
	if tt.crash {
		...
		chain, err = NewBlockChain(db, nil, params.MainnetChainConfig, engine, vm.Config{}, nil, nil)
		if err != nil {
