...
type Tracer interface {
	...
	CaptureState(env *CVM, pc uint64, op OpCode, energy, cost uint64, memory *Memory, stack *Stack, rStack *ReturnStack, contract *Contract, depth int, err error) error
...
func (l *StructLogger) CaptureState(env *CVM, pc uint64, op OpCode, energy, cost uint64, memory *Memory, stack *Stack, rStack *ReturnStack, contract *Contract, depth int, err error) error {
	...
	log := StructLog{pc, op, energy, cost, mem, memory.Len(), stck, rstack, storage, depth, env.StateDB.GetRefund(), err}
