...
func (in *CVMInterpreter) Run(contract *Contract, input []byte, readOnly bool) (ret []byte, err error) {
	...
	if in.cfg.Debug {
		defer func() {
			if err != nil {
				if !logged {
					in.cfg.Tracer.CaptureState(in.cvm, pcCopy, op, energyCopy, cost, mem, stack, in.returnData, contract, in.cvm.depth, err)
				} else {
	...
	for atomic.LoadInt32(&in.cvm.abort) == 0 {
		...
		if in.cfg.Debug {
			in.cfg.Tracer.CaptureState(in.cvm, pc, op, energyCopy, cost, mem, stack, in.returnData, contract, in.cvm.depth, err)
			logged = true
		}
