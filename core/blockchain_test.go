...
func newCanonical(engine consensus.Engine, n int, full bool) (xcbdb.Database, *BlockChain, error) {
	...
	blockchain, _ := NewBlockChain(db, nil, params.MainnetChainConfig, engine, vm.Config{}, nil, nil)
...
func testReorgShort(t *testing.T, full bool) {
	...
	testReorg(t, easy, diff, 788288, full)
}
...
func testReorg(t *testing.T, first, second []int64, td int64, full bool) {
	...
	easyBlocks, _ := GenerateChain(params.MainnetChainConfig, blockchain.CurrentBlock(), cryptore.NewFaker(), db, len(first), func(i int, b *BlockGen) {
		b.OffsetTime(first[i])
	})
	diffBlocks, _ := GenerateChain(params.MainnetChainConfig, blockchain.CurrentBlock(), cryptore.NewFaker(), db, len(second), func(i int, b *BlockGen) {
...
func TestLightVsFastVsFullChainHeads(t *testing.T) {
	...
	var (
		...
		address = key.Address()
		funds   = big.NewInt(1000000000)
		gspec   = &Genesis{Config: params.MainnetChainConfig, Alloc: GenesisAlloc{address: {Balance: funds}}}
		genesis = gspec.MustCommit(gendb)
	)
	height := uint64(1024)
...
func TestChainTxReorgs(t *testing.T) {
	var (
		...
		gspec   = &Genesis{
			...
			EnergyLimit: 3141592,
			Alloc: GenesisAlloc{
				key1.Address(): {Balance: big.NewInt(1000000)},
				key2.Address(): {Balance: big.NewInt(1000000)},
				key3.Address(): {Balance: big.NewInt(1000000)},
			},
		}
	...
	postponed, _ := types.SignTx(types.NewTransaction(0, key1.Address(), big.NewInt(1000), params.TxEnergy, nil, nil), signer, key1)
	swapped, _ := types.SignTx(types.NewTransaction(1, key1.Address(), big.NewInt(1000), params.TxEnergy, nil, nil), signer, key1)
	...
	chain, _ := GenerateChain(gspec.Config, genesis, cryptore.NewFaker(), db, 3, func(i int, gen *BlockGen) {
		switch i {
		case 0:
			pastDrop, _ = types.SignTx(types.NewTransaction(gen.TxNonce(key2.Address()), key2.Address(), big.NewInt(1000), params.TxEnergy, nil, nil), signer, key2)

			gen.AddTx(pastDrop)  // This transaction will be dropped in the fork from below the split point
			gen.AddTx(postponed) // This transaction will be postponed till block #3 in the fork

		case 2:
			freshDrop, _ = types.SignTx(types.NewTransaction(gen.TxNonce(key2.Address()), key2.Address(), big.NewInt(1000), params.TxEnergy, nil, nil), signer, key2)
	...
	chain, _ = GenerateChain(gspec.Config, genesis, cryptore.NewFaker(), db, 5, func(i int, gen *BlockGen) {
		switch i {
		case 0:
			pastAdd, _ = types.SignTx(types.NewTransaction(gen.TxNonce(key3.Address()), key3.Address(), big.NewInt(1000), params.TxEnergy, nil, nil), signer, key3)
		...
		case 2:
			...
			freshAdd, _ = types.SignTx(types.NewTransaction(gen.TxNonce(key3.Address()), key3.Address(), big.NewInt(1000), params.TxEnergy, nil, nil), signer, key3)
			gen.AddTx(freshAdd) // This transaction will be added exactly at reorg time

		case 3:
			futureAdd, _ = types.SignTx(types.NewTransaction(gen.TxNonce(key3.Address()), key3.Address(), big.NewInt(1000), params.TxEnergy, nil, nil), signer, key3)
...
func TestLogReorgs(t *testing.T) {
	var (
		...
		gspec   = &Genesis{Config: params.MainnetChainConfig, Alloc: GenesisAlloc{key1.Address(): {Balance: big.NewInt(10000000000000)}}}
		genesis = gspec.MustCommit(db)
		signer  = types.NewNucleusSigner(gspec.Config.NetworkID)
	)
	...
	chain, _ := GenerateChain(params.MainnetChainConfig, genesis, cryptore.NewFaker(), db, 2, func(i int, gen *BlockGen) {
		if i == 1 {
			tx, err := types.SignTx(types.NewContractCreation(gen.TxNonce(key1.Address()), new(big.Int), 1000000, new(big.Int), code), signer, key1)
			if err != nil {
	...
	chain, _ = GenerateChain(params.MainnetChainConfig, genesis, cryptore.NewFaker(), db, 3, func(i int, gen *BlockGen) {})
...
func TestLogRebirth(t *testing.T) {
	...
	chain, _ := GenerateChain(params.MainnetChainConfig, genesis, engine, db, 2, func(i int, gen *BlockGen) {
		if i == 1 {
	...
	forkChain, _ := GenerateChain(params.MainnetChainConfig, genesis, engine, db, 2, func(i int, gen *BlockGen) {
		if i == 1 {
			tx, err := types.SignTx(types.NewContractCreation(gen.TxNonce(key1.Address()), new(big.Int), 1000000, new(big.Int), logCode), signer, key1)
			if err != nil {
				t.Fatalf("failed to create tx: %v", err)
	...
	newBlocks, _ := GenerateChain(params.MainnetChainConfig, chain[len(chain)-1], engine, db, 1, func(i int, gen *BlockGen) {})
...
func TestSideLogRebirth(t *testing.T) {
	...
	sideChain, _ := GenerateChain(params.MainnetChainConfig, genesis, cryptore.NewFaker(), db, 2, func(i int, gen *BlockGen) {
		if i == 1 {
			tx, err := types.SignTx(types.NewContractCreation(gen.TxNonce(key1.Address()), new(big.Int), 1000000, new(big.Int), logCode), signer, key1)
			if err != nil {
				t.Fatalf("failed to create tx: %v", err)
	...
	newBlocks, _ := GenerateChain(params.MainnetChainConfig, sideChain[len(sideChain)-1], cryptore.NewFaker(), db, 1, func(i int, gen *BlockGen) {})
	if _, err := blockchain.InsertChain(newBlocks); err != nil {
		t.Fatalf("failed to insert forked chain: %v", err)
	}
	checkLogEvents(t, newLogCh, rmLogsCh, 1, 0)
...
func TestReorgSideEvent(t *testing.T) {
	var (
		...
		gspec   = &Genesis{
			Config: params.MainnetChainConfig,
			Alloc:  GenesisAlloc{key1.Address(): {Balance: big.NewInt(10000000000000)}},
		}
		genesis = gspec.MustCommit(db)
		signer  = types.NewNucleusSigner(gspec.Config.NetworkID)
	)
	...
	replacementBlocks, _ := GenerateChain(gspec.Config, genesis, cryptore.NewFaker(), db, 4, func(i int, gen *BlockGen) {
		tx, err := types.SignTx(types.NewContractCreation(gen.TxNonce(key1.Address()), new(big.Int), 1000000, new(big.Int), nil), signer, key1)
		if i == 2 {
			gen.OffsetTime(-9)
		}
		if err != nil {
...
func TestCIP155Transition(t *testing.T) {
	...
	blocks, _ := GenerateChain(gspec.Config, genesis, cryptore.NewFaker(), db, 4, func(i int, block *BlockGen) {
		...
		switch i {
		case 0:
			tx, err = basicTx(types.NewNucleusSigner(big.NewInt(1)))
			if err != nil {
				t.Fatal(err)
			}
			block.AddTx(tx)
	...
	_, err := blockchain.InsertChain(blocks)
	if err != types.ErrInvalidNetworkId {
		t.Error("expected error:", types.ErrInvalidNetworkId)
	}
}
...
//
// https://github.com/core-coin/go-core/v2/pull/15941
func TestBlockchainHeaderchainReorgConsistency(t *testing.T) {
	...
	db := rawdb.NewMemoryDatabase()
	genesis := new(Genesis).MustCommit(db)
	blocks, _ := GenerateChain(params.MainnetChainConfig, genesis, engine, db, 64, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{1}) })
	...
	for i := 0; i < len(forks); i++ {
		...
		fork, _ := GenerateChain(params.MainnetChainConfig, parent, engine, db, 1, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{2}) })
		forks[i] = fork[0]
	}
	...
	chain, err := NewBlockChain(diskdb, nil, params.MainnetChainConfig, engine, vm.Config{}, nil, nil)
	if err != nil {
...
func TestTrieForkGC(t *testing.T) {
	...
	db := rawdb.NewMemoryDatabase()
	genesis := new(Genesis).MustCommit(db)
	blocks, _ := GenerateChain(params.MainnetChainConfig, genesis, engine, db, 2*TriesInMemory, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{1}) })
...
func TestLargeReorgTrieGC(t *testing.T) {
	...
	shared, _ := GenerateChain(params.MainnetChainConfig, genesis, engine, db, 64, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{1}) })
	original, _ := GenerateChain(params.MainnetChainConfig, shared[len(shared)-1], engine, db, 2*TriesInMemory, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{2}) })
	competitor, _ := GenerateChain(params.MainnetChainConfig, shared[len(shared)-1], engine, db, 2*TriesInMemory+1, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{3}) })
...
//
// Details at:
//   - https://github.com/core-coin/go-core/v2/issues/18977
//   - https://github.com/core-coin/go-core/v2/pull/18988
func TestLowDiffLongChain(t *testing.T) {
	...
	blocks, _ := GenerateChain(params.MainnetChainConfig, genesis, engine, db, 6*TriesInMemory, func(i int, b *BlockGen) {
	...
	parent := blocks[10]
	fork, _ := GenerateChain(params.MainnetChainConfig, parent, engine, db, 8*TriesInMemory, func(i int, b *BlockGen) {
...
func testSideImport(t *testing.T, numCanonBlocksInSidechain, blocksBetweenCommonAncestorAndPruneblock int) {
	...
	blocks, _ := GenerateChain(params.MainnetChainConfig, genesis, engine, db, 2*TriesInMemory, nil)
	diskdb := rawdb.NewMemoryDatabase()
	new(Genesis).MustCommit(diskdb)
	chain, err := NewBlockChain(diskdb, nil, params.MainnetChainConfig, engine, vm.Config{}, nil, nil)
	if err != nil {
	...
	fork, _ := GenerateChain(params.MainnetChainConfig, parent, engine, db, 2*TriesInMemory, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{2})
	})
...
func testInsertKnownChainData(t *testing.T, typ string) {
	...
	blocks, receipts := GenerateChain(params.MainnetChainConfig, genesis, engine, db, 32, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{1}) })
	// A longer chain but total difficulty is lower.
	blocks2, receipts2 := GenerateChain(params.MainnetChainConfig, blocks[len(blocks)-1], engine, db, 65, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{1}) })
	// A shorter chain but total difficulty is higher.
	blocks3, receipts3 := GenerateChain(params.MainnetChainConfig, blocks[len(blocks)-1], engine, db, 64, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{1})
	...
	chain, err := NewBlockChain(chaindb, nil, params.MainnetChainConfig, engine, vm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
...
func getLongAndShortChains() (*BlockChain, []*types.Block, []*types.Block, error) {
	...
	longChain, _ := GenerateChain(params.MainnetChainConfig, genesis, engine, db, 80, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{1})
	})
	diskdb := rawdb.NewMemoryDatabase()
	new(Genesis).MustCommit(diskdb)
	...
	heavyChain, _ := GenerateChain(params.MainnetChainConfig, parent, engine, db, 75, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{2})
		b.OffsetTime(-9)
	})
...
func TestTransactionIndices(t *testing.T) {
	...
	l := uint64(0)
	chain, err := NewBlockChain(ancientDb, nil, params.MainnetChainConfig, cryptore.NewFaker(), vm.Config{}, nil, &l)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	headers := make([]*types.Header, len(blocks))
...
func benchmarkLargeNumberOfValueToNonexisting(b *testing.B, numTxs, numBlocks int, recipientFn func(uint64) common.Address, dataFn func(uint64) []byte) {
	addr, err := common.HexToAddress("cb21000000000000000000000000000000000000aaaa")
	if err != nil {
		b.Error(err)
	}
	var (
		...
		gspec          = Genesis{
			Config: params.MainnetChainConfig,
			Alloc: GenesisAlloc{
				testBankKey.Address(): {Balance: bankFunds},
				addr: {
	...
	shared, _ := GenerateChain(params.MainnetChainConfig, genesis, engine, db, numBlocks, blockGenerator)
	b.StopTimer()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		// Import the shared chain and the original canonical one
...
func TestDeleteCreateRevert(t *testing.T) {
	var (
		...
		key, _  = crypto.UnmarshalPrivateKeyHex("856a9af6b0b651dd2f43b5e12193652ec1701c4da6f1c0d2a366ac4b9dabc9433ef09e41ca129552bd2c029086d9b03604de872a3b3432041f")
		address = key.Address()
		funds   = big.NewInt(1000000000)
		gspec   = &Genesis{
			Config: params.MainnetChainConfig,
	...
	blocks, _ := GenerateChain(params.MainnetChainConfig, genesis, engine, db, 1, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{1})
		// One transaction to AAAA
		tx, _ := types.SignTx(types.NewTransaction(0, aa,
			big.NewInt(0), 50000, big.NewInt(1), nil), types.NewNucleusSigner(params.MainnetChainConfig.NetworkID), key)
		b.AddTx(tx)
		// One transaction to BBBB
		tx, _ = types.SignTx(types.NewTransaction(1, bb,
			big.NewInt(0), 100000, big.NewInt(1), nil), types.NewNucleusSigner(params.MainnetChainConfig.NetworkID), key)
		b.AddTx(tx)
	})
	// Import the canonical chain
	diskdb := rawdb.NewMemoryDatabase()
...
func TestDeleteRecreateSlots(t *testing.T) {
	bb, err := common.HexToAddress("cb08000000000000000000000000000000000000bbbb")
	if err != nil {
		t.Error(err)
	}
	var (
	...
	blocks, _ := GenerateChain(params.MainnetChainConfig, genesis, engine, db, 1, func(i int, b *BlockGen) {
		...
		tx, _ := types.SignTx(types.NewTransaction(0, aa,
			big.NewInt(0), 50000, big.NewInt(1), nil), types.NewNucleusSigner(big.NewInt(1)), key)
		b.AddTx(tx)
		// One transaction to BB, to recreate AA
		tx, _ = types.SignTx(types.NewTransaction(1, bb,
			big.NewInt(0), 100000, big.NewInt(1), nil), types.NewNucleusSigner(big.NewInt(1)), key)
		b.AddTx(tx)
	})
	// Import the canonical chain
	diskdb := rawdb.NewMemoryDatabase()
	gspec.MustCommit(diskdb)
	chain, err := NewBlockChain(diskdb, nil, params.MainnetChainConfig, engine, vm.Config{
		Debug:  true,
...
func TestDeleteRecreateAccount(t *testing.T) {
	...
	blocks, _ := GenerateChain(params.MainnetChainConfig, genesis, engine, db, 1, func(i int, b *BlockGen) {
		...
		tx, _ = types.SignTx(types.NewTransaction(1, aa,
			big.NewInt(1), 100000, big.NewInt(1), nil), types.NewNucleusSigner(big.NewInt(1)), key)
		b.AddTx(tx)
	})
	// Import the canonical chain
	diskdb := rawdb.NewMemoryDatabase()
...
func TestDeleteRecreateSlotsAcrossManyBlocks(t *testing.T) {
	...
	blocks, _ := GenerateChain(params.MainnetChainConfig, genesis, engine, db, 150, func(i int, b *BlockGen) {
...
//     with 0.001 core (thus created but no code)
...
func TestInitThenFailCreateContract(t *testing.T) {
	...
	genesis := gspec.MustCommit(db)
	nonce := uint64(0)
	blocks, _ := GenerateChain(params.MainnetChainConfig, genesis, engine, db, 4, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{1})
