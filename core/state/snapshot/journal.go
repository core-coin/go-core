...
func loadAndParseLegacyJournal(db xcbdb.KeyValueStore, base *diskLayer) (snapshot, journalGenerator, error) {
...
func loadAndParseJournal(db xcbdb.KeyValueStore, base *diskLayer) (snapshot, journalGenerator, error) {
	...
	// not existent, e.g. the disk layer is generating while that the Gocore
	...
	// It can happen that Gocore crashes without persisting the latest
	// diff journal.
...
func loadSnapshot(diskdb xcbdb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash, recovery bool) (snapshot, error) {
	...
	//
	// Possible scenario: Gocore was crashed without persisting journal and then
