...
// SnapshotTree is an Core state snapshot tree. It consists of one persistent
...
func New(diskdb xcbdb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash, async bool, recovery bool) *Tree {
...
func diffToDisk(bottom *diffLayer) *diskLayer {
	...
	for hash := range bottom.destructSet {
		...
		for it.Next() {
			if key := it.Key(); len(key) == 65 || len(key) == 57 { // TODO(raisty): Yuck, we should move this into the iterator
				batch.Delete(key)
