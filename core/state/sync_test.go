...
func makeTestState() (Database, common.Hash, []*testAccount) {
	...
	for i := byte(0); i < 96; i++ {
		...
		if i%5 == 0 {
			for j := byte(0); j < 5; j++ {
				hash := crypto.SHA3Hash([]byte{i, i, i, i, i, j, j})
...
// account array.
func checkStateAccounts(t *testing.T, db xcbdb.Database, root common.Hash, accounts []*testAccount) {
...
func checkTrieConsistency(db xcbdb.Database, root common.Hash) error {
...
func checkStateConsistency(db xcbdb.Database, root common.Hash) error {
	...
	for len(hashQueue)+len(pathQueue) > 0 {
		...
		for i, path := range pathQueue {
			if len(path) == 1 {
				...
				results[len(hashQueue)+i] = trie.SyncResult{Hash: crypto.SHA3Hash(data), Data: data}
			} else {
				var acc Account
...
func TestIncompleteStateSync(t *testing.T) {
	...
	for _, acc := range srcAccounts {
		if len(acc.code) > 0 {
			isCode[crypto.SHA3Hash(acc.code)] = struct{}{}
		}
	}
