...
type ChainIndexer struct {
	chainDb  xcedb.Database      // Chain database to index the data from
	indexDb  xcedb.Database      // Prefixed table-view of the db to write index metadata into
...
func NewChainIndexer(chainDb xcedb.Database, indexDb xcedb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
