...
func pricedTransaction(nonce uint64, energylimit uint64, energyprice *big.Int, key *eddsa.PrivateKey) *types.Transaction {
	tx, _ := types.SignTx(types.NewTransaction(nonce, common.Address{}, big.NewInt(100), energylimit, energyprice, nil), types.NewNucleusSigner(params.AllCryptoreProtocolChanges.ChainID), key)
	return tx
}
...
func pricedDataTransaction(nonce uint64, energylimit uint64, energyprice *big.Int, key *eddsa.PrivateKey, bytes uint64) *types.Transaction {
	...
	tx, _ := types.SignTx(types.NewTransaction(nonce, common.Address{}, big.NewInt(0), energylimit, energyprice, data), types.NewNucleusSigner(params.AllCryptoreProtocolChanges.ChainID), key)
	return tx
}
...
func deriveSender(tx *types.Transaction) (common.Address, error) {
	return types.Sender(types.NewNucleusSigner(params.AllCryptoreProtocolChanges.ChainID), tx)
}
...
func TestTransactionNegativeValue(t *testing.T) {
	...
	tx, _ := types.SignTx(types.NewTransaction(0, common.Address{}, big.NewInt(-1), 100, big.NewInt(1), nil), types.NewNucleusSigner(params.AllCryptoreProtocolChanges.ChainID), key)
	from, _ := deriveSender(tx)
	pool.currentState.AddBalance(from, big.NewInt(1))
