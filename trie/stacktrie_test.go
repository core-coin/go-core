...
func genTxs(num uint64) (types.Transactions, error) {
	key, err := crypto.HexToEDDSA("deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef")
	if err != nil {
		return nil, err
	}
	pub := eddsa.Ed448DerivePublicKey(*key)
	var addr = crypto.PubkeyToAddress(pub)
	newTx := func(i uint64) (*types.Transaction, error) {
		signer := types.MakeSigner(big.NewInt(18))
