...
func newTestTransport(rpub *eddsa.PublicKey, fd net.Conn, dialDest *eddsa.PublicKey) transport {
...
func (c *testTransport) doEncHandshake(prv *eddsa.PrivateKey) (*eddsa.PublicKey, error) {
	return c.rpub, nil
}
...
func startTestServer(t *testing.T, remoteKey *eddsa.PublicKey, pf func(*Peer)) *Server {
	...
	server := &Server{
		Config:      config,
		newPeerHook: pf,
		newTransport: func(fd net.Conn, dialDest *eddsa.PublicKey) transport {
...
func TestServerAtCap(t *testing.T) {
	...
	newconn := func(id enode.ID) *conn {
		fd, _ := net.Pipe()
		tx := newTestTransport(&pub, fd, nil)
...
func TestServerPeerLimits(t *testing.T) {
	...
	srv := &Server{
		...
		newTransport: func(fd net.Conn, dialDest *eddsa.PublicKey) transport { return tp },
	}
	if err := srv.Start(); err != nil {
...
func TestServerSetupConn(t *testing.T) {
	...
	for i, test := range tests {
		t.Run(test.wantCalls, func(t *testing.T) {
			...
			srv := &Server{
				Config:       cfg,
				newTransport: func(fd net.Conn, dialDest *eddsa.PublicKey) transport { return test.tt },
				log:          cfg.Logger,
			}
			if !test.dontstart {
				if err := srv.Start(); err != nil {
...
func (c *setupTransport) doEncHandshake(prv *eddsa.PrivateKey) (*eddsa.PublicKey, error) {
