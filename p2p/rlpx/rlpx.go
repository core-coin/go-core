...
func NewConn(conn net.Conn, dialDest *crypto.PublicKey) *Conn {
	return &Conn{
		dialDest: dialDest,
		conn:     conn,
	}
...
func (c *Conn) Handshake(prv *crypto.PrivateKey) (*crypto.PublicKey, error) {
	var (
		sec Secrets
		err error
	)
...
const (
	...
	pubLen = 57 //  pubkey
	shaLen = 64 // hash length (for nonce etc)
...
type encHandshake struct {
	initiator            bool
	remote               *crypto.PublicKey  // remote-pubk
	initNonce, respNonce []byte             // nonce
	randomPrivKey        *crypto.PrivateKey // ecdhe-random
	remoteRandomPub      *crypto.PublicKey  // ecdhe-random-pubk
}
...
func receiverEncHandshake(conn io.ReadWriter, prv *crypto.PrivateKey) (s Secrets, err error) {
...
func (h *encHandshake) handleAuthMsg(msg *authMsgV4, prv *crypto.PrivateKey) error {
...
func (h *encHandshake) staticSharedSecret(prv *crypto.PrivateKey) ([]byte, error) {
...
//
// prv is the local client's private key.
func initiatorEncHandshake(conn io.ReadWriter, prv *crypto.PrivateKey, remote *crypto.PublicKey) (s Secrets, err error) {
...
func (h *encHandshake) makeAuthMsg(prv *crypto.PrivateKey) (*authMsgV4, error) {
	...
	copy(msg.InitiatorPubkey[:], prv.PublicKey()[:])
...
func readHandshakeMsg(msg interface{}, plainSize int, prv *crypto.PrivateKey, r io.Reader) ([]byte, error) {
	...
	dec, err := ecies.Decrypt(prv, buf[2:], nil, prefix)
	if err != nil {
		return buf, err
	}
...
// importPublicKey unmarshals  public keys.
func importPublicKey(pubKey []byte) (*crypto.PublicKey, error) {
...
func exportPubkey(pub *crypto.PublicKey) []byte {
	if pub == nil {
		panic("nil pubkey")
	}
	return pub[:]
