...
func TestProtocolHandshake(t *testing.T) {
	var (
		...
		hs0     = &protoHandshake{Version: 3, ID: crypto.FromEDDSAPub(&pub0)[1:], Caps: []Cap{{"a", 0}, {"b", 2}}}

		prv1    = newkey()
		pub1    = eddsa.Ed448DerivePublicKey(*prv1)
		hs1     = &protoHandshake{Version: 3, ID: crypto.FromEDDSAPub(&pub1)[1:], Caps: []Cap{{"c", 1}, {"d", 3}}}
	...
	go func() {
		defer wg.Done()
		defer fd0.Close()
		frame := newRLPX(fd0, &pub1)
		rpubkey, err := frame.doEncHandshake(prv0)
		if err != nil {
			t.Errorf("dial side enc handshake failed: %v", err)
			return
		}
		if !reflect.DeepEqual(rpubkey, &pub1) {
			t.Errorf("dial side remote pubkey mismatch: got %v, want %v", rpubkey, &pub1)
			return
		}
	...
	go func() {
		...
		if !reflect.DeepEqual(rpubkey, &pub0) {
			t.Errorf("listen side remote pubkey mismatch: got %v, want %v", rpubkey, &pub0)
			return
		}
