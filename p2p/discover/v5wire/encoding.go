...
func NewCodec(ln *enode.LocalNode, key *crypto.PrivateKey, clock mclock.Clock) *Codec {
	c := &Codec{
		sha256:    sha3.New256(),
		localnode: ln,
		privkey:   key,
...
func (c *Codec) makeHandshakeAuth(toID enode.ID, addr string, challenge *Whoareyou) (*handshakeAuthData, *session, error) {
	...
	var remotePubkey = new(crypto.PublicKey)
	if err := challenge.Node.Load((*enode.Ed448)(remotePubkey)); err != nil {
		return nil, nil, fmt.Errorf("can't find ed448 key for recipient")
	}
	ephkey, err := c.sc.ephemeralKeyGen(crand.Reader)
	if err != nil {
	...
	// Create session keys.
	sec := deriveKeys(sha3.New256, ephkey, remotePubkey, c.localnode.ID(), challenge.Node.ID(), cdata)
	if sec == nil {
...
func (c *Codec) decodeHandshake(fromAddr string, head *Header) (n *enode.Node, auth handshakeAuthData, s *session, err error) {
	...
	session := deriveKeys(sha3.New256, c.privkey, ephkey, auth.h.SrcID, c.localnode.ID(), cdata)
