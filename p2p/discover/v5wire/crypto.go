...
func EncodePubkey(key *crypto.PublicKey) []byte {
	return key[:]
}
...
func makeIDSignature(hash hash.Hash, key *crypto.PrivateKey, challenge, ephkey []byte, destID enode.ID) ([]byte, error) {
...
// ed448raw is an unparsed ed448 public key ENR entry.
type ed448raw []byte
...
func verifyIDSignature(hash hash.Hash, sig []byte, n *enode.Node, challenge, ephkey []byte, destID enode.ID) error {
	...
	case "v4":
		...
		if n.Load(&pubkey) != nil {
			return errors.New("no ed448 public key in record")
		}
		input := idNonceHash(hash, challenge, ephkey, destID)
...
func deriveKeys(hash hashFn, priv *crypto.PrivateKey, pub *crypto.PublicKey, n1, n2 enode.ID, challenge []byte) *session {
...
func ecdh(privkey *crypto.PrivateKey, pubkey *crypto.PublicKey) []byte {
	shared := crypto.ComputeSecret(privkey, pubkey)
	return shared
}
