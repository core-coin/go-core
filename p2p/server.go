...
type Server struct {
	...
	newTransport func(net.Conn, *eddsa.PublicKey) transport
...
type transport interface {
	// The two handshakes.
	doEncHandshake(prv *eddsa.PrivateKey) (*eddsa.PublicKey, error)
...
func (srv *Server) setupDiscovery() error {
	...
	if srv.NAT != nil {
		if !realaddr.IP.IsLoopback() {
			srv.loopWG.Add(1)
			go func() {
				nat.Map(srv.NAT, srv.quit, "udp", realaddr.Port, realaddr.Port, "core discovery")
				srv.loopWG.Done()
			}()
		}
	}
...
func (srv *Server) setupConn(c *conn, flags connFlag, dialDest *enode.Node) error {
	...
	if id := c.node.ID(); !bytes.Equal(crypto.SHA3(phs.ID), id[:]) {
