...
var (
	...
	maxQueuedHeaders         = 32 * 1024                    // [xcb/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	stateDB    xcbdb.Database  // Database to state sync into (and deduplicate via)
	...
	// Channels
	headerCh      chan dataPack        // [xcb/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [xcb/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [xcb/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [xcb/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [xcb/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [xcb/62] Channel to feed the header processor new tasks

	// for stateFetcher
	stateSyncStart chan *stateSync
	trackStateReq  chan *stateReq
	stateCh        chan dataPack // [xcb/63] Channel receiving inbound node state data
...
func New(checkpoint uint64, stateDb xcbdb.Database, stateBloom *trie.SyncBloom, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "xcb", p.version, "head", hash, "td", td, "mode", d.mode)
