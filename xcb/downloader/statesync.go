...
type stateSync struct {
	...
	keccak hash.Hash  // SHA3 hasher to verify deliveries with
...
func (s *stateSync) commit(force bool) error {
	if !force && s.bytesUncommitted < xcbdb.IdealBatchSize {
		return nil
	}
	start := time.Now()
	...
	for _, p := range peers {
		...
		if len(nodes)+len(codes) > 0 {
			...
			case s.d.trackStateReq <- req:
				req.peer.FetchNodeData(append(nodes, codes...)) // Unified retrieval under xcb/6x
			case <-s.cancel:
			case <-s.d.cancelCh:
			}
		}
