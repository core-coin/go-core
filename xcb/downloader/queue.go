...
type fetchRequest struct {
	...
	From    uint64          // [xcb/62] Requested chain element index (used for skeleton fills only)
	Headers []*types.Header // [xcb/62] Requested headers, sorted by request order
...
type queue struct {
	...
	headerHead      common.Hash                    // [xcb/62] Hash of the last queued header to verify order
	headerTaskPool  map[uint64]*types.Header       // [xcb/62] Pending header retrieval tasks, mapping starting indexes to skeleton headers
	headerTaskQueue *prque.Prque                   // [xcb/62] Priority queue of the skeleton indexes to fetch the filling headers for
	headerPeerMiss  map[string]map[uint64]struct{} // [xcb/62] Set of per-peer header batches known to be unavailable
	headerPendPool  map[string]*fetchRequest       // [xcb/62] Currently pending header retrieval operations
	headerResults   []*types.Header                // [xcb/62] Result cache accumulating the completed headers
	headerProced    int                            // [xcb/62] Number of headers already processed from the results
	headerOffset    uint64                         // [xcb/62] Number of the first header in the result cache
	headerContCh    chan bool                      // [xcb/62] Channel to notify when header download finishes
