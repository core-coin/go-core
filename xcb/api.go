...
// PublicCoreAPI provides an API to access Core full node-related
// information.
...
// NetworkId is the CIP-155 replay-protection chain id for the current core chain config.
...
// PrivateAdminAPI is the collection of Core full node-related APIs
...
// admin methods of the Core service.
func NewPrivateAdminAPI(xcb *Core) *PrivateAdminAPI {
	return &PrivateAdminAPI{xcb: xcb}
}
...
func (api *PrivateAdminAPI) ExportChain(file string, first *uint64, last *uint64) (bool, error) {
	...
	if first != nil && last == nil {
		head := api.xcb.BlockChain().CurrentHeader().Number.Uint64()
		last = &head
	}
	...
	if first != nil {
		if err := api.xcb.BlockChain().ExportN(writer, *first, *last); err != nil {
			return false, err
		}
	} else if err := api.xcb.BlockChain().Export(writer); err != nil {
		return false, err
	}
	return true, nil
}
...
func (api *PrivateAdminAPI) ImportChain(file string) (bool, error) {
	...
	for batch := 0; ; batch++ {
		...
		if hasAllBlocks(api.xcb.BlockChain(), blocks) {
			blocks = blocks[:0]
			continue
		}
		// Import the batch and reset the buffer
		if _, err := api.xcb.BlockChain().InsertChain(blocks); err != nil {
...
// PublicDebugAPI is the collection of Core full node APIs exposed
...
// related public debug methods of the Core service.
func NewPublicDebugAPI(xcb *Core) *PublicDebugAPI {
	return &PublicDebugAPI{xcb: xcb}
}
...
func (api *PublicDebugAPI) DumpBlock(blockNr rpc.BlockNumber) (state.Dump, error) {
	if blockNr == rpc.PendingBlockNumber {
		...
		_, stateDb := api.xcb.miner.Pending()
	...
	if blockNr == rpc.LatestBlockNumber {
		...
		block = api.xcb.blockchain.GetBlockByNumber(uint64(blockNr))
	}
	if block == nil {
...
// PrivateDebugAPI is the collection of Core full node APIs exposed over
...
// private debug methods of the Core service.
func NewPrivateDebugAPI(xcb *Core) *PrivateDebugAPI {
	return &PrivateDebugAPI{xcb: xcb}
}

// Preimage is a debug API function that returns the preimage for a sha3 hash, if known.
func (api *PrivateDebugAPI) Preimage(ctx context.Context, hash common.Hash) (hexutil.Bytes, error) {
	if preimage := rawdb.ReadPreimage(api.xcb.ChainDb(), hash); preimage != nil {
		return preimage, nil
	}
...
func (api *PrivateDebugAPI) GetBadBlocks(ctx context.Context) ([]*BadBlockArgs, error) {
	...
	for i, block := range blocks {
		...
		if results[i].Block, err = xcbapi.RPCMarshalBlock(block, true, true, big.NewInt(int64(api.xcb.config.NetworkId))); err != nil {
...
func (api *PrivateDebugAPI) GetModifiedAccountsByNumber(startNum uint64, endNum *uint64) ([]common.Address, error) {
	...
	startBlock = api.xcb.blockchain.GetBlockByNumber(startNum)
	if startBlock == nil {
	...
	if endNum == nil {
		endBlock = startBlock
		startBlock = api.xcb.blockchain.GetBlockByHash(startBlock.ParentHash())
		if startBlock == nil {
			return nil, fmt.Errorf("block %x has no parent", endBlock.Number())
		}
	} else {
		endBlock = api.xcb.blockchain.GetBlockByNumber(*endNum)
		if endBlock == nil {
...
func (api *PrivateDebugAPI) GetModifiedAccountsByHash(startHash common.Hash, endHash *common.Hash) ([]common.Address, error) {
	var startBlock, endBlock *types.Block
	startBlock = api.xcb.blockchain.GetBlockByHash(startHash)
	if startBlock == nil {
	...
	if endHash == nil {
		...
		endBlock = api.xcb.blockchain.GetBlockByHash(*endHash)
		if endBlock == nil {
...
func (api *PrivateDebugAPI) getModifiedAccounts(startBlock, endBlock *types.Block) ([]common.Address, error) {
	...
	triedb := api.xcb.BlockChain().StateCache().TrieDB()
