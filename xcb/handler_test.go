...
// Tests that post xcb protocol handshake, clients perform a mutual checkpoint
...
func testCheckpointChallenge(t *testing.T, syncmode downloader.SyncMode, checkpoint bool, timeout bool, empty bool, match bool, drop bool) {
	...
	peer, _ := newTestPeer("peer", xcb63, pm, true)
	defer peer.close()
...
func testBroadcastBlock(t *testing.T, totalPeers, broadcastExpected int) {
	...
	for i := 0; i < totalPeers; i++ {
		peer, _ := newTestPeer(fmt.Sprintf("peer %d", i), xcb63, pm, true)
		defer peer.close()
	...
	for _, peer := range peers {
		go func(p *testPeer) {
			if err := p2p.ExpectMsg(p.app, NewBlockMsg, &newBlockData{Block: chain[0], TD: big.NewInt(0x2000)}); err != nil {
				errCh <- err
			} else {
				doneCh <- struct{}{}
			}
...
func TestBroadcastMalformedBlock(t *testing.T) {
	...
	// propagation
	source, _ := newTestPeer("source", xcb63, pm, true)
	defer source.close()

	sink, _ := newTestPeer("sink", xcb63, pm, true)
	defer sink.close()
