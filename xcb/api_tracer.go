...
import (
	...
	"github.com/core-coin/go-core/v2/xcb/tracers"
)
...
func (api *PrivateDebugAPI) TraceChain(ctx context.Context, start, end rpc.BlockNumber, config *TraceConfig) (*rpc.Subscription, error) {
	...
	case rpc.PendingBlockNumber:
		from = api.xcb.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		from = api.xcb.blockchain.CurrentBlock()
	default:
		from = api.xcb.blockchain.GetBlockByNumber(uint64(start))
	}
	switch end {
	case rpc.PendingBlockNumber:
		to = api.xcb.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		to = api.xcb.blockchain.CurrentBlock()
	default:
		to = api.xcb.blockchain.GetBlockByNumber(uint64(end))
	}
...
func (api *PrivateDebugAPI) traceChain(ctx context.Context, start, end *types.Block, config *TraceConfig) (*rpc.Subscription, error) {
	...
	database := state.NewDatabaseWithConfig(api.xcb.ChainDb(), &trie.Config{Cache: 16, Preimages: true})

	if number := start.NumberU64(); number > 0 {
		start = api.xcb.blockchain.GetBlock(start.ParentHash(), start.NumberU64()-1)
		if start == nil {
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range tasks {
				...
				blockCtx := core.NewCVMBlockContext(task.block.Header(), api.xcb.blockchain, nil)
	...
	go func() {
		...
		for number = start.NumberU64() + 1; number <= end.NumberU64(); number++ {
			...
			block := api.xcb.blockchain.GetBlockByNumber(number)
			if block == nil {
			...
			_, _, _, err := api.xcb.blockchain.Processor().Process(block, statedb, vm.Config{})
			if err != nil {
				failed = err
				break
			}
...
func (api *PrivateDebugAPI) TraceBlockByNumber(ctx context.Context, number rpc.BlockNumber, config *TraceConfig) ([]*txTraceResult, error) {
	...
	switch number {
	case rpc.PendingBlockNumber:
		block = api.xcb.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		block = api.xcb.blockchain.CurrentBlock()
	default:
		block = api.xcb.blockchain.GetBlockByNumber(uint64(number))
	}
...
func (api *PrivateDebugAPI) TraceBlockByHash(ctx context.Context, hash common.Hash, config *TraceConfig) ([]*txTraceResult, error) {
	block := api.xcb.blockchain.GetBlockByHash(hash)
	if block == nil {
...
func (api *PrivateDebugAPI) TraceBadBlock(ctx context.Context, hash common.Hash, config *TraceConfig) ([]*txTraceResult, error) {
	blocks := api.xcb.blockchain.BadBlocks()
...
func (api *PrivateDebugAPI) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	if err := api.xcb.engine.VerifyHeader(api.xcb.blockchain, block.Header(), true); err != nil {
		return nil, err
	}
	parent := api.xcb.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
	...
	blockCtx := core.NewCVMBlockContext(block.Header(), api.xcb.blockchain, nil)
	for th := 0; th < threads; th++ {
		pend.Add(1)
		go func() {
			defer pend.Done()
	...
	for i, tx := range txs {
		...
		vmenv := vm.NewCVM(blockCtx, txContext, statedb, api.xcb.blockchain.Config(), vm.Config{})
...
func (api *PrivateDebugAPI) standardTraceBlockToFile(ctx context.Context, block *types.Block, config *StdTraceConfig) ([]string, error) {
	...
	var (
		signer = types.MakeSigner(api.xcb.blockchain.Config().NetworkID)
		dumps  []string
		vmctx  = core.NewCVMBlockContext(block.Header(), api.xcb.blockchain, nil)
	)
	for i, tx := range block.Transactions() {
		...
		vmenv := vm.NewCVM(vmctx, txContext, statedb, api.xcb.blockchain.Config(), vmConf)
...
func (api *PrivateDebugAPI) computeStateDB(block *types.Block, reexec uint64) (*state.StateDB, error) {
	...
	statedb, err := api.xcb.blockchain.StateAt(block.Root())
	if err == nil {
		return statedb, nil
	}
	...
	for i := uint64(0); i < reexec; i++ {
		block = api.xcb.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
		if block == nil {
			break
		}
	...
	for block.NumberU64() < origin {
		...
		if block = api.xcb.blockchain.GetBlockByNumber(block.NumberU64() + 1); block == nil {
...
func (api *PrivateDebugAPI) TraceTransaction(ctx context.Context, hash common.Hash, config *TraceConfig) (interface{}, error) {
	...
	tx, blockHash, _, index := rawdb.ReadTransaction(api.xcb.ChainDb(), hash)
	if tx == nil {
	...
	// Retrieve the block
	block := api.xcb.blockchain.GetBlockByHash(blockHash)
	if block == nil {
...
func (api *PrivateDebugAPI) TraceCall(ctx context.Context, args xcbapi.CallArgs, blockNrOrHash rpc.BlockNumberOrHash, config *TraceConfig) (interface{}, error) {
	...
	statedb, header, err := api.xcb.APIBackend.StateAndHeaderByNumberOrHash(ctx, blockNrOrHash)
	if err != nil {
	...
	// Execute the trace
	msg := args.ToMessage(api.xcb.APIBackend.RPCEnergyCap())
	vmctx := core.NewCVMBlockContext(header, api.xcb.blockchain, nil)
	return api.traceTx(ctx, msg, vmctx, statedb, config)
}
...
func (api *PrivateDebugAPI) traceTx(ctx context.Context, message core.Message, vmctx vm.BlockContext, statedb *state.StateDB, config *TraceConfig) (interface{}, error) {
	...
	vmenv := vm.NewCVM(vmctx, txContext, statedb, api.xcb.blockchain.Config(), vm.Config{Debug: true, Tracer: tracer})
...
func (api *PrivateDebugAPI) computeTxEnv(block *types.Block, txIndex int, reexec uint64) (core.Message, vm.BlockContext, *state.StateDB, error) {
	...
	for idx, tx := range block.Transactions() {
		...
		msg, _ := tx.AsMessage(signer)
		txContext := core.NewCVMTxContext(msg)
		context := core.NewCVMBlockContext(block.Header(), api.xcb.blockchain, nil)
		if idx == txIndex {
			return msg, context, statedb, nil
		}
		// Not yet the searched for transaction, execute on top of the current state
		vmenv := vm.NewCVM(context, txContext, statedb, api.xcb.blockchain.Config(), vm.Config{})
