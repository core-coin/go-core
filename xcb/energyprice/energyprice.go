...
var DefaultMaxPrice = big.NewInt(500 * params.Nucle)
...
// NewOracle returns a new energyprice oracle which can recommend suitable
// energyprice for newly created transaction.
func NewOracle(backend OracleBackend, params Config) *Oracle {
	blocks := params.Blocks
	if blocks < 1 {
		blocks = 1
		log.Warn("Sanitizing invalid energyprice oracle sample blocks", "provided", params.Blocks, "updated", blocks)
	}
	percent := params.Percentile
	if percent < 0 {
		percent = 0
		log.Warn("Sanitizing invalid energyprice oracle sample percentile", "provided", params.Percentile, "updated", percent)
	}
	if percent > 100 {
		percent = 100
		log.Warn("Sanitizing invalid energyprice oracle sample percentile", "provided", params.Percentile, "updated", percent)
	...
	if maxPrice == nil || maxPrice.Int64() <= 0 {
		...
		log.Warn("Sanitizing invalid energyprice oracle price cap", "provided", params.MaxPrice, "updated", maxPrice)
	}
	return &Oracle{
		backend:     backend,
...
// SuggestPrice returns a energyprice so that newly created transaction can
...
func (gpo *Oracle) SuggestPrice(ctx context.Context) (*big.Int, error) {
	...
	for sent < gpo.checkBlocks && number > 0 {
		go gpo.getBlockPrices(ctx, types.MakeSigner(gpo.backend.ChainConfig().NetworkID), number, sampleNumber, result, quit)
		sent++
		exp++
		number--
	}
...
func (gpo *Oracle) getBlockPrices(ctx context.Context, signer types.Signer, blockNum uint64, limit int, result chan getBlockPricesResult, quit chan struct{}) {
	...
	copy(txs, blockTxs)
	sort.Sort(transactionsByEnergyPrice(txs))

	var prices []*big.Int
	for _, tx := range txs {
		sender, err := types.Sender(signer, tx)
		if err == nil && sender != block.Coinbase() {
			prices = append(prices, tx.EnergyPrice())
			if len(prices) >= limit {
				break
			}
		}
