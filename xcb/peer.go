...
// PeerInfo represents a short summary of the Core sub-protocol metadata known
...
//
// This method is legacy support for initial transaction exchange in xcb/64 and
// prior. For xcb/65 and higher use SendPooledTransactionHashes.
...
// Handshake executes the xcb protocol handshake, negotiating version number,
...
func (p *peer) String() string {
	return fmt.Sprintf("Peer %s [%s]", p.id,
		fmt.Sprintf("xcb/%2d", p.version),
	)
}

// peerSet represents the collection of active peers currently participating in
// the Core sub-protocol.
type peerSet struct {
