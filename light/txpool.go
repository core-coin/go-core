...
// to the XCB network. The implementations of the functions should be non-blocking.
//
...
func NewTxPool(config *params.ChainConfig, chain *LightChain, relay TxRelayBackend) *TxPool {
	pool := &TxPool{
		config:      config,
		signer:      types.NewNucleusSigner(config.NetworkID),
...
func (pool *TxPool) validateTx(ctx context.Context, tx *types.Transaction) error {
	...
	// Validate the transaction recipient and it's sig. Throw
	// if the from fields is invalid.
	if from, err = types.Sender(pool.signer, tx); err != nil {
		return core.ErrInvalidRecipientOrSig
	}
	...
	energy, err := core.IntrinsicEnergy(tx.Data(), tx.To() == nil)
	if err != nil {
		return err
	}
	if tx.Energy() < energy {
