...
func (ks *KeyStore) SignTx(account *Account, tx *Transaction, networkID *BigInt) (*Transaction, error) {
	if networkID == nil { // Null passed from mobile app
		networkID = new(BigInt)
	}
	signed, err := ks.keystore.SignTx(account.account, tx.tx, networkID.bigint)
	if err != nil {
		return nil, err
	}
...
func (ks *KeyStore) SignTxPassphrase(account *Account, passphrase string, tx *Transaction, networkID *BigInt) (*Transaction, error) {
	if networkID == nil { // Null passed from mobile app
		networkID = new(BigInt)
	}
	signed, err := ks.keystore.SignTxWithPassphrase(account.account, passphrase, tx.tx, networkID.bigint)
	if err != nil {
		return nil, err
	}
	return &Transaction{signed}, nil
