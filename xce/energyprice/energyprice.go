...
// SuggestPrice returns the recommended energy price.
func (gpo *Oracle) SuggestPrice(ctx context.Context) (*big.Int, error) {
	...
	for sent < gpo.checkBlocks && blockNum > 0 {
		go gpo.getBlockPrices(ctx, types.MakeSigner(), blockNum, ch)
		sent++
		exp++
		blockNum--
	}
...
func (t transactionsByEnergyPrice) Less(i, j int) bool { return t[i].EnergyPrice().Cmp(t[j].EnergyPrice()) < 0 }
...
func (gpo *Oracle) getBlockPrices(ctx context.Context, signer types.Signer, blockNum uint64, ch chan getBlockPricesResult) {
	...
	for _, tx := range txs {
		...
		if err == nil && sender != block.Coinbase() {
			ch <- getBlockPricesResult{tx.EnergyPrice(), nil}
			return
		}
	}
