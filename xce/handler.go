...
func NewProtocolManager(config *params.ChainConfig, checkpoint *params.TrustedCheckpoint, mode downloader.SyncMode, networkID uint64, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb xcedb.Database, cacheLimit int, whitelist map[uint64]common.Hash) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Core peer", "peer", id)

	// Unregister the peer from the downloader and Core peer set
...
func (pm *ProtocolManager) Start(maxPeers int) {
	...
	// start sync handlers
	go pm.syncer()
	go pm.txsyncLoop64() // TODO(karalabe): Legacy initial tx echange, drop with xce/64.
}

func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Core protocol")
	...
	log.Info("Core protocol stopped")
}
...
// handle is the callback invoked to manage the life cycle of an xce peer. When
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Core peer connected", "name", p.Name())

	// Execute the Core handshake
	var (
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash(), forkid.NewID(pm.blockchain), pm.forkFilter); err != nil {
		p.Log().Debug("Core handshake failed", "err", err)
		return err
	}
	// Register the peer locally
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Core peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Core message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case p.version >= xce63 && msg.Code == GetNodeDataMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= xce63 && msg.Code == NodeDataMsg:
	...
	case p.version >= xce63 && msg.Code == GetReceiptsMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= xce63 && msg.Code == ReceiptsMsg:
	...
	case msg.Code == NewPooledTransactionHashesMsg && p.version >= xce65:
	...
	case msg.Code == GetPooledTransactionsMsg && p.version >= xce65:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case msg.Code == TransactionMsg || (msg.Code == PooledTransactionsMsg && p.version >= xce65):
...
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Core network ID (1=Frontier, 2=Morden, Testnet=3, Rinkeby=4)
