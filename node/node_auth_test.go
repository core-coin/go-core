...
func (at *authTest) Run(t *testing.T) {
	...
	err = cl.CallContext(ctx, &x, "xcb_helloWorld")
...
func TestAuthEndpoints(t *testing.T) {
	...
	// Gocore must read it from a file, and does not support in-memory JWT secrets, so we create a temporary file.
	...
	conf := &Config{
		...
		WSModules:   []string{"xcb", "engine"},
		HTTPModules: []string{"xcb", "engine"}}
	node, err := New(conf)
	if err != nil {
