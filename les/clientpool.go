...
import (
	...
	lps "github.com/core-coin/go-core/v2/les/lespay/server"
...
const (
	...
	// or sync some data.
	//
	// todo(raisty) make it configurable. It can be the option of
	// free trial time!
...
func newClientPool(ns *nodestate.NodeStateMachine, lespayDb xcbdb.Database, minCap uint64, connectedBias time.Duration, clock mclock.Clock, removePeer func(enode.ID)) *clientPool {
	pool := &clientPool{
		ns:                  ns,
