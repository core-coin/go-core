...
func prepare(n int, backend *backends.SimulatedBackend) {
	var (
		...
		signer = types.NewNucleusSigner(backend.Blockchain().Config().NetworkID)
	)
	for i := 0; i < n; i++ {
		switch i {
		case 0:
...
func testIndexers(db xcbdb.Database, odr light.OdrBackend, config *light.IndexerConfig, disablePruning bool) []*core.ChainIndexer {
