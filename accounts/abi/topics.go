...
//
// Note, dynamic types cannot be reconstructed since they get mapped to SHA3
...
func parseTopicWithSetter(fields Arguments, topics []common.Hash, setter func(Argument, interface{})) error {
	...
	for i, arg := range fields {
		...
		case StringTy, BytesTy, SliceTy, ArrayTy:
			// Array types (including strings and bytes) have their SHA3 hashes stored in the topic- not a hash
			// whose bytes can be decoded to the actual value- so the best we can do is retrieve that hash
			reconstr = topics[i]
		case FunctionTy:
			if garbage := binary.BigEndian.Uint16(topics[i][0:2]); garbage != 0 {
				return fmt.Errorf("bind: got improperly encoded function type, got %v", topics[i].Bytes())
			}
			if garbage := binary.BigEndian.Uint32(topics[i][2:6]); garbage != 0 {
				return fmt.Errorf("bind: got improperly encoded function type, got %v", topics[i].Bytes())
			}
			var tmp [26]byte
			copy(tmp[:], topics[i][6:32])
			reconstr = tmp
		default:
			var err error
