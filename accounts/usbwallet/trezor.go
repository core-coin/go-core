...
import (
	...
	"github.com/core-coin/go-core/accounts/usbwallet/trezor"
...
// or whether the Core app was not started on it.
...
// Core address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.CoreAddress)
	if _, err := w.trezorExchange(&trezor.CoreGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.CoreTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.CoreTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
